    ...
    return (
        <>
            <List.Section>
                ...
                <List.Item
                    title="Asleep"
                    right={() => <Text>{getHours(data.totalSleepTime)}h {getMinutes(data.totalSleepTime)}m</Text>}
                    description="Average time spent asleep"
                    contentStyle={styles.listItem}
                />
                <List.Item
                    title="In bed"
                    right={() => <Text>{getHours(data.timeInBed)}h {getMinutes(data.timeInBed)}m</Text>}
            ...
            <List.Section style={styles.listSection}>
                ...
                <List.Item
                    title="Light sleep"
                    right={() => <Text>{getHours(data.timeLightSleep)}h {getMinutes(data.timeLightSleep)}m</Text>}
                    description="Average time in light sleep"
                    contentStyle={styles.listItem}
                />
                <List.Item
                    title="Deep sleep"
                    right={() => <Text>{getHours(data.timeDeepSleep)}h {getMinutes(data.timeDeepSleep)}m</Text>}
                    description="Average time in deep sleep"
                    contentStyle={styles.listItem}
                />
                <List.Item
                    title="REM sleep"
                    right={() => <Text>{getHours(data.timeRemSleep)}h {getMinutes(data.timeRemSleep)}m</Text>}
