...
export default function Overview() {
  ...
  const [last7Data, setLast7Data] = useState<SleepSessionAvgData | undefined>(
    undefined
  );
  const [last30Data, setLast30Data] = useState<SleepSessionAvgData | undefined>(
    undefined
  );
  const [allTimeData, setAllTimeData] = useState<
    SleepSessionAvgData | undefined
  >(undefined);
  ...
  useEffect(() => {
    ...
    const getLast30DaysPromise = getLast30Days().then((records) =>
      handleLast30DaysResult(records)
    );

    const getBeforeNowPromise = getBeforeNow().then((records) =>
      handleBeforeNowResult(records)
    );

    Promise.all([
      getLast7DaysPromise,
      getLast30DaysPromise,
      getBeforeNowPromise,
    ])
